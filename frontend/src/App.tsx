...
export default function App() {
  ...
  const submit = (values: FormValues) => {
    setSubmitted(values); setStatus('loading')
    window.setTimeout(() => setStatus('success'), reduceMotion ? 250 : 1350)
  }
  ...
  return <div className="min-h-screen overflow-hidden bg-background text-foreground">
    ...
    <main id="top" className="relative z-10">
      <section className="mx-auto max-w-5xl px-5 pb-16 pt-16 text-center sm:px-8 sm:pb-24 sm:pt-24 lg:pt-28">
        ...
        <motion.div className="mt-11 text-left sm:mt-14" initial={reduceMotion ? false : { opacity:0,y:24 }} animate={{ opacity:1,y:0 }} transition={{ duration:.7,delay:.12 }}><ConciergeForm form={form} onSubmit={submit} onPromptSelect={choosePrompt} isLoading={status==='loading'} /></motion.div>
      </section>
      <AnimatePresence mode="wait">
        {status==='idle' ? <motion.section key="idle" exit={{ opacity:0 }} className="border-y border-white/[.05] bg-white/[.015]"><div className="mx-auto grid max-w-6xl gap-8 px-5 py-12 sm:grid-cols-3 sm:px-8 lg:px-12">
        ...
        </div></motion.section> : <motion.section key="results" initial={{ opacity:0,y:20 }} animate={{ opacity:1,y:0 }} exit={{ opacity:0 }} className="mx-auto max-w-7xl px-5 pb-24 sm:px-8 lg:px-12" aria-live="polite" aria-busy={status==='loading'}>
          <div className="mb-9 flex flex-col gap-5 border-t border-white/[.07] pt-10 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">Your private screening</p><h2 className="mt-2 font-display text-3xl sm:text-4xl">{status==='loading' ? 'Curating your watchlist…' : 'Selected for this moment'}</h2><p className="mt-3 max-w-2xl truncate text-sm text-muted-foreground">“{submitted?.userPrompt}”</p></div>{status==='success' ? <Button variant="outline" onClick={() => submitted && submit(submitted)}><RotateCcw className="size-4" aria-hidden="true" />Regenerate</Button> : null}</div>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{status==='loading' ? Array.from({ length:submitted?.count ?? 3 },(_,index)=><MovieSkeleton key={index} />) : mockMovies.slice(0,submitted?.count ?? 3).map((movie,index)=><MovieCard key={movie.title} movie={movie} index={index} />)}</div>
        </motion.section>}
      </AnimatePresence>
    </main>
